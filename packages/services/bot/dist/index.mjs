...
var getBotNodeFromStartingMessage = async (startingMessage) => {
  ...
  if (cache.has(cacheKey)) {
    ...
    try {
      const response = await axios.get(`${botServiceConfig_default.getConfig("baseUrl")}/admin/bot/search`, {
        params: {
          perPage: 5,
          page: 1,
          match: true,
...
var getBotNodeFromId = async (botId) => {
  ...
  if (cache.has(cacheKey)) {
    ...
    try {
      const response = await axios.get(`${botServiceConfig_default.getConfig("baseUrl")}/admin/bot/${botId}`);
      if (response.data) {
        console.log(
...
export {
  botServiceConfig_default as botServiceConfig,
  getBotIdFromBotName,
  getBotNameByBotID,
  getBotNodeFromId,
  getBotNodeFromName,

...
export class HttpPatchTransformer implements ITransformer {
  ...
  ///     body: Body for the HTTP PATCH request. If not provided, `XMessage.transformer.metaData.httpBody` will be used. (optional)
  ...
  async transform(xmsg: XMessage): Promise<XMessage> {
    ...
    console.log("HTTP PATCH transformer called.");
    ...
    console.log("HTTP PATCH url-", this.config.url)
    console.log("HTTP PATCH body -", typeof this.config.body === 'string' ? this.config.body : JSON.stringify(this.config.body ?? {}));
    console.log("HTTP PATCH headers -", new Headers(this.config.headers))

    if (!this.config.url) {
      this.telemetryLogger.sendErrorTelemetry(xmsg, '`url` not defined in HTTP_PATCH transformer');
      throw new Error('`url` not defined in HTTP_PATCH transformer');
    }
    await fetch(this.config.url, {
      ...
      .catch((ex) => {
        this.telemetryLogger.sendErrorTelemetry(xmsg, `PATCH request failed. Reason: ${ex}`);
        console.error(`PATCH request failed. Reason: ${ex}`);
        throw ex;
      });

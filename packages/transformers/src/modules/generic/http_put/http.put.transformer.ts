...
export class HttpPutTransformer implements ITransformer {
  ...
  ///     body: Body for the HTTP PUT request. If not provided, `XMessage.transformer.metaData.httpBody` will be used. (optional)
  ...
  async transform(xmsg: XMessage): Promise<XMessage> {
    ...
    console.log("HTTP PUT url-", this.config.url)
    console.log("HTTP PUT body -", typeof this.config.body === 'string' ? this.config.body : JSON.stringify(this.config.body ?? {}));
    console.log("HTTP PUT headers -", new Headers(this.config.headers))

    if (!this.config.url) {
      this.telemetryLogger.sendErrorTelemetry(xmsg, '`url` not defined in HTTP_PUT transformer');
      throw new Error('`url` not defined in HTTP_PUT transformer');
    }
    await fetch(this.config.url, {
      ...
      .catch((ex) => {
        this.telemetryLogger.sendErrorTelemetry(xmsg, `PUT request failed. Reason: ${ex}`);
